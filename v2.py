...
# 교통사고 시작
lane_change_times = [1, 2, 3, 4]
speed_change_times = [3, 4, 5, 6]
changed_lanes = [-3, -4, -5, -6]
changed_speeds = [20, 30, 40, 50]
...
for i in range(num_of_car):
    actions_content = actions_content + f"""            <Private entityRef="Vehicle{i + 1}">
               <PrivateAction>
                  <TeleportAction>
                     <Position>
                        <LanePosition roadId="0" laneId="{car_pos_lanes[i]}" offset="0" s="{car_pos_fars[i]}"/>
                     </Position>
                  </TeleportAction>
               </PrivateAction>
               <PrivateAction>
...
cut_in_act = ""
for i in range(num_of_car):
    cut_in_act = cut_in_act + f"""            <ManeuverGroup maximumExecutionCount="1" name="CutInSequence{i+1}">
               <Actors selectTriggeringEntities="false">
                  <EntityRef entityRef="Vehicle{i+1}"/>
               </Actors>
               <Maneuver name="CutInManeuver{i+1}">
                  <Event name="CutInEvent{i+1}" priority="overwrite">
                        ...
                        <PrivateAction>
                           <LateralAction>
                              <LaneChangeAction> <!-- 횡방향 행동 중 차선 변경 행동을 의미힌다. -->
                                 ...
                                 <LaneChangeTarget>
                                    <AbsoluteTargetLane value="{changed_lanes[i]}"/>
                                 </LaneChangeTarget>
                              </LaneChangeAction>
                           </LateralAction>
                        </PrivateAction>
                     ...
                     <StartTrigger>
                        <ConditionGroup>
                           <Condition name="CutInActStart{i+1}" delay="0" conditionEdge="rising">
                              <ByValueCondition>
                                 <SimulationTimeCondition value="{lane_change_times[i]}" rule="greaterThan"/>
                              </ByValueCondition>
                           </Condition>
                        </ConditionGroup>
                     </StartTrigger>
            ...
            <ManeuverGroup maximumExecutionCount="1" name="Speedup{i+1}">
                  <Actors selectTriggeringEntities="false">
                     <EntityRef entityRef="Vehicle{i+1}"/>
                  </Actors>
                  <Maneuver name="SpeedupManeuver{i+1}">
                     <Event name="SpeedupEvent{i+1}" priority="overwrite">
                           ...
                           <PrivateAction>
                              <LongitudinalAction>
                                 <SpeedAction>
                                    <SpeedActionDynamics dynamicsShape="step" dynamicsDimension="time" value="0.0"/>
                                    <SpeedActionTarget>
                                       <AbsoluteTargetSpeed value="{float(changed_speeds[i]) / 3.6}"/>
                                    </SpeedActionTarget>
                                 </SpeedAction>
                              </LongitudinalAction>
                           </PrivateAction>
                        ...
                        <StartTrigger>
                           <ConditionGroup>
                              <Condition name="CutInActStart" delay="0" conditionEdge="rising">
                                 <ByValueCondition>
                                    <SimulationTimeCondition value="{speed_change_times[i]}" rule="greaterThan"/>
                                 </ByValueCondition>
                              </Condition>
                           </ConditionGroup>
                        </StartTrigger>
