...
with open('content/depth/3vehicle/depth_dic_bounding_box_가리는부분제거_3vehicle.json', 'r') as pos_file:
    pos_dic = json.load(pos_file)
with open('content/차선을float로표현_v9/exact_postion_3vehicle.json', 'r') as lane_file:
...
for i in range(num_of_car):
    ...
    ratio_ini, lane_ini = math.modf(lane_dic[car][0][1])
    w_pos = lanes_w[int(lane_ini)]*(ratio_ini-0.5)
    car_pos_lanes.append([-lane_ini-1, w_pos])
    # car_pos_fars에 집어넣는다.
...
for i in range(num_of_car):
    ...
    for frame in range(frame_num-1):
        ...
        ratio, lane = math.modf(lane_dic[car][frame+1][1])
        w_pos = lanes_w[int(lane)]*(ratio-0.5)
        car_poses.append([-lane-1, w_pos])
