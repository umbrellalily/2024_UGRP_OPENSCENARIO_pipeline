...
with open('content/depth/3vehicle/depth_dic_bounding_box_범위수정.json', 'r') as pos_file:
    pos_dic = json.load(pos_file)
with open('content/차선단위 변경_v8/lane_change_midpoints_3vehicle.json', 'r') as lane_file:
...
for i in range(num_of_car):
    ...
    # car_pos_lanes에 넣는다
    car_pos_lanes.append([-lane_dic[car][0]["start_lane"]-1, 0])

num_of_speed_change = [frame_num - 1 for _ in range(num_of_car)]
num_of_lane_change = []
speed_change_times = [[i * 1/frame_per_sec for i in range(frame_num-1)] for _ in range(num_of_car)]
speed_change_values = [[1/frame_per_sec for i in range(frame_num-1)] for _ in range(num_of_car)]
lane_change_times = []
lane_change_values = []
changed_lanes = []
changed_speeds = []
...
for i in range(num_of_car):
    ...
    for frame in range(frame_num-1):
        ...
        car_speeds.append((pos_dic[car][frame+1]-pos_dic[car][frame])/(1/frame_per_sec))
    changed_speeds.append(car_speeds)

    lane_change_times_temp = []
    lane_change_values_temp = []
    changed_lanes_temp = []
    if lane_dic[car][0]["end_lane"] == -1:
        num_of_lane_change.append(0)
    else:
        for event in lane_dic[car]:
            lane_change_times_temp.append(event["midpoint_time"])
        del lane_change_times_temp[-1]

        previous_midpoint = 0
        for event in lane_dic[car]:
            lane_change_values_temp.append(event["midpoint_time"] - previous_midpoint)
            previous_midpoint = event["midpoint_time"]
        del lane_change_values_temp[0]

        for event in lane_dic[car]:
            if event["end_lane"] != -1:
                changed_lanes_temp.append([-event["end_lane"]-1, 0])
        num_of_lane_change.append(len(lane_dic[car]) - 1)
    lane_change_times.append(lane_change_times_temp)
    lane_change_values.append(lane_change_values_temp)
    changed_lanes.append(changed_lanes_temp)

print(lane_change_times)
print(lane_change_values)
print(changed_lanes)
                            ...
                            <PrivateAction>
                               <LateralAction>
                                  <LaneChangeAction> <!-- 횡방향 행동 중 차선 변경 행동을 의미힌다. -->
                                     <LaneChangeActionDynamics dynamicsShape="linear" value="{lane_change_values[i][j]}" dynamicsDimension="time"/><!-- ***이 값은 얼마나 앞에서 차선을 변경하는 가이다 -->
                                     <LaneChangeTarget>
                         ...
                         <StartTrigger>
                            <ConditionGroup>
                               <Condition name="CutInActStart{distinct_num}" delay="0" conditionEdge="rising">
                                  <ByValueCondition>
                                     <SimulationTimeCondition value="{lane_change_times[i][j]+0.1}" rule="greaterThan"/>
                                  </ByValueCondition>
                               </Condition>
                            </ConditionGroup>
                         </StartTrigger>
